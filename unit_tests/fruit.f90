...
!
! Change Log:
!     1.  Modified by David C. Stauffer in August 2019 to support quad precision float
!         comparisons (for 0d values).
!     2.  Modified by David C. Stauffer to explicitly declare all reals for single, double or quad, so that
!         you can compile with any default option for the real kind.
...
contains
  ...
  function quadEqual (number1, number2 ) result (resultValue)
    real(qp), intent (in) :: number1, number2
    real(dp) :: epsilon
    logical :: resultValue
...
module fruit
  ...
  !====== begin of generated interface ======
    ...
    module procedure assert_eq_quad_
    module procedure assert_eq_quad_in_range_
  ...
  !====== begin of generated interface ======
    ...
    module procedure assert_not_equals_quad_
    module procedure assert_not_equals_quad_in_range_
  !====== end of generated interface ======
...
!$omp end critical (FRUIT_OMP_DEALLOCATE_MESSAGE_ARRAY)
  ...
  function  case_delta_t()
    ...
    if (time_rate > 0) then
      delta_t = real(case_time_to - case_time_from, kind=sp) / real(time_rate, kind=sp)
      if (delta_t < 0) then
        delta_t = delta_t + real(time_max, kind=sp) / real(time_rate, kind=sp)
      endif
    else
      delta_t = 0._sp
    endif
...
999 continue
  ...
  subroutine assert_eq_complex_(var1, var2, message)
    ...
    complex(kind=dp), intent (in) :: var1, var2
...
&(aimag(var1) > aimag(var2))) then
  ...
  subroutine assert_eq_1d_complex_(var1, var2, n, message)
    integer, intent (in) :: n
    integer              :: i
    complex(kind=dp), intent (in) :: var1(n), var2(n)
...
&(aimag(var1(i)) > aimag(var2(i)))) then
  ...
  subroutine assert_eq_2d_complex_(var1, var2, n, m, message)
    integer, intent (in) :: n, m
    integer              :: i, j
    complex(kind=dp), intent (in) :: var1(n, m), var2(n, m)
...
&(aimag(var1(i, j)) > aimag(var2(i, j)))) then
  ...
  !------ 0d_quad ------
  subroutine assert_eq_quad_(var1, var2, message)

    real(qp), intent (in) :: var1, var2
    ...
    call add_success
  end subroutine assert_eq_quad_

  !------ 0d_double ------
  subroutine assert_eq_quad_in_range_(var1, var2, delta, message)

    real(qp), intent (in) :: var1, var2
    real(dp), intent (in) :: delta ! TODO: write another with this as qp?
    character(len = *), intent (in), optional :: message
    ...
    call add_success
  end subroutine assert_eq_quad_in_range_
...
&(aimag(var1(i, j)) > aimag(var2(i, j)))) then
  ...
  !------ 0d_quad ------
  subroutine assert_not_equals_quad_(var1, var2, message)
    ...
    call add_success
  end subroutine assert_not_equals_quad_

  !------ 0d_quad ------
  subroutine assert_not_equals_quad_in_range_(var1, var2, delta, message)

    real(qp), intent (in) :: var1, var2
    real(dp), intent (in) :: delta ! TODO: could make another version with this as a qp?
    character(len = *), intent (in), optional :: message
    logical :: same_so_far
    ...
    call add_success
  end subroutine assert_not_equals_quad_in_range_
